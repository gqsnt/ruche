...
pub async fn get_encounters(summoner_id: i32, page_number: i32, filters: Option<MatchFiltersSearch>, search_summoner: Option<String>) -> Result<SummonerEncounters, ServerFnError> {
    ...
    inner_get_encounters(&db, summoner_id, page_number, filters.unwrap_or_default(), search_summoner).await.map_err(|_| ServerFnError::new("Error fetching champions"))
}

#[cfg(feature = "ssr")]
async fn inner_get_encounters(
    db: &PgPool,
    summoner_id: i32,
    page: i32,
