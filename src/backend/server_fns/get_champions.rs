...
pub async fn get_champions(summoner_id: i32, filters: Option<MatchFiltersSearch>) -> Result<Vec<ChampionStats>, ServerFnError> {
    ...
    inner_get_champions(&db, summoner_id, filters.unwrap_or_default()).await.map_err(|_| ServerFnError::new("Error fetching champions"))
}

#[cfg(feature = "ssr")]
async fn inner_get_champions(
    db: &PgPool,
    summoner_id: i32,
    ...
    let results = query.build_query_as::<ChampionStatsModel>().fetch_all(db).await.unwrap();
