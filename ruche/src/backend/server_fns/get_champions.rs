...
pub mod ssr {
    ...
    pub async fn inner_get_champions(
        ...
        if let Some(queue_id) = filters.queue_id {
            ...
            query.push(sql_filter);
            query.push_bind(Queue::try_from(queue_id).unwrap().id() as i32);
        }
        ...
        Ok(query
            .build_query_as::<ChampionStatsModel>()
            .fetch_all(db)
            .await?
            .into_iter()
            .map(|champion_stats| {
                let win_rate = (champion_stats.total_wins as f32
                    / champion_stats.total_matches as f32)
                    * 100.0;
                ChampionStats {
                    ...
                    avg_kill_participation: champion_stats
                    ...
                    avg_gold_earned: champion_stats.avg_gold_earned.to_f64().unwrap_or_default()
                        as u32,
                    avg_cs: champion_stats.avg_cs.to_f32().unwrap_or_default() as u32,
                    avg_damage_dealt_to_champions: champion_stats
                        .avg_damage_dealt_to_champions
                        .to_f32()
                        .unwrap_or_default()
                        as u32,
                    avg_damage_taken: champion_stats.avg_damage_taken.to_f32().unwrap_or_default()
                        as u32,
